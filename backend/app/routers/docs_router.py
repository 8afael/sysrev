...
from app.models.models import Review, User, Document, Assignment 
...
from app.services.google_drive_service import delete_file_from_s3, upload_file_to_s3
...
async def delete_document(
    ...
    s3_key = doc.drive_file_id  # guarda antes de apagar o registro

    # Apaga assignments e reviews associados explicitamente
    assignment_result = await db.execute(
        select(Assignment.id).where(Assignment.document_id == document_id)
    )
    assignment_ids = [row[0] for row in assignment_result.all()]

    if assignment_ids:
        await db.execute(delete(Review).where(Review.assignment_id.in_(assignment_ids)))
        await db.execute(delete(Assignment).where(Assignment.id.in_(assignment_ids)))

    await log_audit(db, user.id, "delete", "document", document_id, data_after={"title": doc.title})

    await db.delete(doc)
    await db.commit()

    # Banco é a fonte de verdade: se o S3 falhar aqui, só logamos.
    # O usuário já teve sucesso na exclusão do documento.
    try:
        await delete_file_from_s3(s3_key)
    except HTTPException:
        print(f"⚠️ Documento {document_id} removido do banco, mas falhou ao apagar do S3 (key: {s3_key})")